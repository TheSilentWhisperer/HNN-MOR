...
class DynDataset(Dataset):
    ...
    def __init__(self, file_path, device, data_type="point", split="train", keys=[]):
        ...
        split_idx = {
            "train": (0, int(0.8 * self.nb_traj)),
            "val": (int(0.8 * self.nb_traj), int(0.9 * self.nb_traj)),
            "test": (int(0.9 * self.nb_traj), self.nb_traj)
        }

        self.nb_traj_split = split_idx[split][1] - split_idx[split][0]
        ...
        self.n_data_point = self.traj_len * self.nb_traj_split if data_type == "point" else self.nb_traj_split
