...
if __name__ == "__main__":
    ...
    config = json.load(open("scripts/LNN/config.json"))
    ...
    train_loader, val_loader = DataLoader(train_dataset, batch_size=config["BATCH_SIZE"], shuffle=True), DataLoader(val_dataset, batch_size=config["VAL_BATCH_SIZE"], shuffle=False)

    model = LNN(train_dataset.manifold_dim).to(device)

    #create a directory to save the new run
    if "RUN_ID" in config and os.path.exists(os.path.join("runs", config["RUN_ID"])) and os.path.exists(os.path.join("runs", config["RUN_ID"], "model.pt")):
    ...
    with open(f"scripts/LNN/config.json", "w") as f:
        json.dump(config, f, indent=4)
